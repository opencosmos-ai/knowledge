...
// In this repository the corpus *is* the root — but chunk IDs must not move.
// An ID is `{path}#{slug}`, and Cosmo's citation tokens are literal strings in
// its system prompt (`[ref: knowledge/sources/{work}.md#{slug}]`). Rebasing the
// path would change all ~4,100 IDs, make every existing citation a dead link,
// and cause the stale-ID sync to delete and re-create the whole index. So the
// corpus keeps its historical `knowledge/` prefix in IDs, permanently, and
// independently of which repository the corpus happens to live in.
const KNOWLEDGE_DIR = ROOT_DIR
const CORPUS_PREFIX = 'knowledge'

/** Path as it appears in chunk IDs and citations — always `knowledge/…`. */
function corpusPath(filePath: string): string {
  return join(CORPUS_PREFIX, relative(KNOWLEDGE_DIR, filePath))
}
...
function buildChunks(filePath: string): VectorChunk[] {
  ...
  const relPath = corpusPath(filePath)
...
async function main() {
  ...
  if (dryRun) {
    ...
    const chunks = [
      ...
      ...quoteFiles.flatMap(f => buildQuoteChunks(f)),
    ]
    console.log(`DRY RUN — ${chunks.length} chunks from ${files.length} docs + ${quoteFiles.length} quote files`)
    console.log('Sample IDs:')
    for (const c of chunks.slice(0, 5)) console.log('  ' + c.id)
    if (process.env.DRY_RUN_IDS_OUT) {
      writeFileSync(process.env.DRY_RUN_IDS_OUT, chunks.map(c => c.id).join('\n'))
      console.log(`Wrote ${chunks.length} IDs to ${process.env.DRY_RUN_IDS_OUT}`)
    }
  ...
  if (!vectorUrl || !vectorToken) {
    ...
    console.error('   Add them to .env.local, or set them as Actions secrets.')
    process.exit(1)
  }
  ...
  const files = walkMd(KNOWLEDGE_DIR)
  const quoteFiles = walkQuotes()
  const kaizenFiles: string[] = []  // kaizen is embedded by the cosmo repository
  console.log(
  ...
  for (const file of files) {
    ...
    if (chunks.length > 0) {
      console.log(`  ${corpusPath(file)} → ${chunks.length} chunks`)
    }
  }
  for (const file of kaizenFiles) {
    ...
    allChunks.push(...chunks)
    if (chunks.length > 0) {
      console.log(`  ${corpusPath(file)} → ${chunks.length} kaizen chunks`)
    }
  }
  for (const file of quoteFiles) {
    ...
    allChunks.push(...chunks)
    if (chunks.length > 0) {
      console.log(`  ${corpusPath(file)} → ${chunks.length} quote chunks`)
    }
  }
  ...
  if (shouldSync && !shouldReset) {
    ...
    const owned = existingIds.filter(id => id.startsWith(CORPUS_PREFIX + '/'))
    const foreign = existingIds.length - owned.length
    if (foreign > 0) console.log(`   ${foreign} vector(s) owned by another writer — left untouched.`)
    const stale = owned.filter(id => !seenIds.has(id))
    if (stale.length === 0) {
      console.log(`   Index is in sync (${owned.length} owned vectors, 0 stale).`)
    } else {
      console.log(`   Deleting ${stale.length} stale vector(s) (of ${owned.length} owned)...`)
